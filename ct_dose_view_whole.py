'''
原主程序，功能：
读取CT和剂量NIfTI文件，重采样剂量到CT空间，
提取用户指定的轴位切片，加载RTSTRUCT轮廓并映射到CT空间，
最后在三个轴位切片上叠加剂量分布和轮廓进行可视化展示。
'''
...
import ct_dose_view. ct_dose_origin as fc
...
if __name__ == "__main__":
    ...
    # 重采样
    ct_image, dose_image, dose_on_ct_sitk, dose_on_ct = resample(
        ct_array, ct_origin, ct_spacing, dose_array, dose_origin, dose_spacing,
        ct_affine=ct_img_nib.affine, dose_affine=dose_img_nib.affine
    )
    ...
    # 用RS对应DICOM CT几何参数映射ROI，避免NIfTI affine与RS坐标系差异导致偏移
    roi_ct_origin, roi_ct_spacing, roi_ct_orientation = load_roi_reference_geometry(os.path.join(BASE_DIR, "NPC_401"))
